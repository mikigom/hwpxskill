...
19. **edit_guard 필수 통과**: 기존 문서 편집 시 `edit_guard.py`로 편집 전후 무결성 검증 필수. `--before`(원본)와 `--after`(결과)를 모두 지정하여 구조 변경·텍스트 손상·ID 참조 불일치를 사전 차단
20. **deep 검증 권장**: `validate.py --deep`로 itemCnt 정합성, IDRef 존재 여부, mixed-content 손상, secPr 보존을 추가 검증
...
| `scripts/edit_guard.py` | 편집 전후 무결성 비교 검증 (구조·텍스트·IDRef·mixed-content) |
