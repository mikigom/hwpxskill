...
18. **Mixed Content 주의**: `<hp:t>`는 텍스트와 자식 요소(`<hp:fwSpace/>`, `<hp:lineBreak/>` 등)가 혼재하는 Mixed Content 요소이다. 수동 편집 시에도 내부 공백·줄바꿈을 임의로 변경하지 말 것 — 한컴오피스가 이를 실제 텍스트로 렌더링하여 레이아웃이 깨진다
